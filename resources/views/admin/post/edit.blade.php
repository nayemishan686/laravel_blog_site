                    ...
                    <form action="{{route('post.update',$post->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                                ...
                                <select name="subcategory_id" id="subcategory_id" class="form-control @error('subcategory_id') is-invalid @enderror">
                                    ...
                                    @endphp
                                        ...
                                        @foreach ($subcategory as $subcat)
                                            <option value="{{$subcat->id}}" @if($subcat->id == $post->subcategory_id) selected @endif>--{{$subcat->subcategory_name}}</option>
                                        @endforeach
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control @error ('post_date') is_invalid @enderror" name="post_date" id="post_date" value="{{$post->post_date}}">
                                @error('post_date')
                                    <div class="text text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <textarea name="description"  class="form-control summernote @error ('description') is_invalid @enderror" id="description" cols="30" rows="20">{{$post->description}}</textarea>
                                @error('description')
                                    <div class="text text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input @error ('status') is_invalid @enderror" name="status" id="status" value="1" @if($post->status == 1) checked @endif>
